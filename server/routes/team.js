...
// ── Team Tracker: location sharing & clock-in/out ──
...
// GET /api/team/locations — current pins for the org
// Owners/admins see everyone; other roles see only themselves.
router.get("/locations", async (req, res, next) => {
  try {
    ...
    res.json({ members, canViewAll: canManageTeam(req.userRole) });
  } catch (err) { next(err); }
});

// POST /api/team/clock-in — mark the caller as on the clock
...
// POST /api/team/clock-out — clear clocked-in state and drop the live pin
...
// POST /api/team/location — receive a live position from a clocked-in member
